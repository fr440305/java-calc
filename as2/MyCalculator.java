...
public class MyCalculator {
	...
	public boolean isBalanced (String expression) {
		...
		Stack<Parenthesis> pars = new ArrayStack<Parenthesis> (1 << 8);
	...
	// ...
	private String cleanParenthesis (String infix) {
		...
		do {
			...
			) infix = infix.substring(1, len-1); else return infix;
		} while (true);
	}
	//
	//assume that infix is balanced, neat, and has at least one operator.
	private int idofRootOperator (String infix) {
		// `idof` means index of.
		Operator[] ops = new Operator[1 << 8];
		int ops_size = 0;
	...
	public String infixToPostfix (String infix) {
		...
		return (
			infixToPostfix(infix_left) + " " +
			infixToPostfix(infix_right) + " " +
			infix.charAt(root_op)
		);
	}
	public double evaluate (String postfix) {
		Stack<Operand> operand_stack = new ArrayStack<Operand>(1 << 8);
